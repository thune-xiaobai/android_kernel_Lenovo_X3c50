...
#if 0 /*mask hpout detect firstly*/
...
static const unsigned int arizona_full_rates[] = {
	11025,
	22050,
	44100,
	88200,
...
static const struct snd_pcm_hw_constraint_list arizona_full_constraint = {
	.count	= ARRAY_SIZE(arizona_full_rates),
	.list	= arizona_full_rates,
};
			   ...
			   struct snd_soc_dai *dai)
{
    ...
    printk("arizona_startup base_rate=%d \n",base_rate);
...
#endif
	constraint = &arizona_full_constraint;
			    ...
			    unsigned int Fref, bool sync)
{
	...
	arizona_fll_dbg(fll, "Fref=%u Fout=%u,  Fvco=%dHz,  N=%x THETA=%x LAMBDA=%x,  FRATIO=%x(%d) OUTDIV=%x REFCLK_DIV=%x,  GAIN=%d\n", 
			Fref, fll->fout, target, cfg->n, cfg->theta, cfg->lambda, cfg->fratio, cfg->fratio, cfg->outdiv, cfg->refdiv, cfg->gain);
...
static int arizona_wait_for_fll(struct arizona_fll *fll, bool requested)
{
    ...
    /*here it should never happen,then we should try to test two registers */
	arizona_fll_warn(fll, "Timed out waiting for lock val=%x status=%d\n",val,status);

    /*Then check id register to check spi ARIZONA_SOFTWARE_RESET*/
    val = 0; 
    regmap_read(arizona->regmap, ARIZONA_SOFTWARE_RESET, &val);
    arizona_fll_warn(fll, "Timed out waiting then check registers chipId val=%x\n",val);

    /*Then check id register to check spi ARIZONA_SOFTWARE_RESET*/
    val = 0; 
    regmap_read(arizona->regmap, ARIZONA_DEVICE_REVISION, &val);
    arizona_fll_warn(fll, "Timed out waiting then check registers chip version val=%x\n",val);
