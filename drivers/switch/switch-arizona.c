...
static bool triple_pole_headphone = false;
static int lartge_micimpedance_delaycheck = 0;
static int large_mic_imp=0;
...
#ifdef CONFIG_SND_SOC_FLORIDA /*add Lenovo headset key*/
...
enum headset_state {
	...
	BIT_HEADSET_UNSUPPORTED  = (1 << 2),
};

static int headphone_switch_state_old;
...
static void arizona_jds_timeout_work(struct work_struct *work)
{
	...
	if((info->state != NULL) && (info->state->timeout!= NULL))
...
void arizona_set_headphone_imp(struct arizona_extcon_info *info, int imp)
{
	...
	dev_dbg(arizona->dev,"Set headphone switch to the state it should be %d\n",headphone_switch_state_old);
        headphone_switch_set(headphone_switch_state_old);
...
int arizona_hpdet_reading(struct arizona_extcon_info *info, int val)
{
	...
	if (info->mic) {
		...
		val += info->arizona->pdata.hpdet_ext_res;
		if(triple_pole_headphone || (large_mic_imp && val < ARIZONA_HPDET_MAX)) 
			arizona_extcon_report(info, BIT_HEADSET_NO_MIC);
		else if (!(large_mic_imp && val >= ARIZONA_HPDET_MAX))
				arizona_extcon_report(info, BIT_HEADSET_UNSUPPORTED);
...
int arizona_micd_low_ohm_start(struct arizona_extcon_info *info) 
{
	int ret = 0;
	info->micd_low_ohm_done = true;
	arizona_extcon_hp_clamp(info, true);
	ret = arizona_micd_start(info);

	return ret;
}
EXPORT_SYMBOL_GPL(arizona_micd_low_ohm_start);
...
void arizona_micd_low_ohm_stop(struct arizona_extcon_info *info)
{      
	info->micd_low_ohm_done = false;
	arizona_extcon_hp_clamp(info, false);
	arizona_micd_stop(info);
}
EXPORT_SYMBOL_GPL(arizona_micd_low_ohm_stop);
...
int arizona_micd_mic_reading(struct arizona_extcon_info *info, int val)
{
	...
	if (val > MICROPHONE_MAX_OHM) {
		large_mic_imp=1;
		if(lartge_micimpedance_delaycheck++ < 4)
		{
			dev_warn(arizona->dev, "Need recheck mic impedance %d\n", lartge_micimpedance_delaycheck);
			msleep(200);
			return -EAGAIN;
		}else
		{
	...
	if (val >= MICROPHONE_MIN_OHM) {
		dev_warn(arizona->dev, "Detected headset\n");
		info->mic = true;
		goto done;
	}
	...
	if( info->micd_low_ohm && !info->micd_low_ohm_done) {
		dev_dbg(arizona->dev, "Starting Low-Ohm-detection \n");   
		ret = arizona_jds_set_state(info, &arizona_micd_low_ohm_mic);
		return 0;
	}
	...
	if(val < 10)
	{
	       ...
	       triple_pole_headphone = true;
	}else
	       dev_dbg(arizona->dev, "Detected OMTP headset\n");
	...
	if (ret < 0) {
		dev_warn(arizona->dev, "HP detect start failed, WHY?\n");
		if (info->mic)
			arizona_extcon_report(info, BIT_HEADSET);
		else
			arizona_extcon_report(info, BIT_HEADSET_NO_MIC);
...
const struct arizona_jd_state arizona_micd_low_ohm_mic = {
       .mode = ARIZONA_ACCDET_MODE_ADC,
       .start = arizona_micd_low_ohm_start,
       .restart = arizona_micd_restart,
       .reading = arizona_micd_mic_reading,
       .stop = arizona_micd_low_ohm_stop,

       .timeout_ms = arizona_micd_mic_timeout_ms,
       .timeout = arizona_micd_mic_timeout,
};
EXPORT_SYMBOL_GPL(arizona_micd_low_ohm_mic);
...
static irqreturn_t arizona_jackdet(int irq, void *data)
{
	...
	if (present) {
		dev_dbg(arizona->dev, "Detected jack val %d, hpdet_acc_id=%d, custom_jd=%lu, antenna_supported=%d, hpdet_moisture_imp=%d, micd_software_compare=%d\n", val, arizona->pdata.hpdet_acc_id, (unsigned long)arizona->pdata.custom_jd, arizona->pdata.antenna_supported, arizona->pdata.hpdet_moisture_imp, arizona->pdata.micd_software_compare);
		headphone_switch_state_old = headphone_switch_state();
		headphone_switch_set(1);
		...
		dev_dbg(arizona->dev, "Detected jack removal, val %d\n", val);

		triple_pole_headphone = false;
		lartge_micimpedance_delaycheck = 0;
		large_mic_imp=0;
		info->num_hpdet_res = 0;
...
static int arizona_extcon_of_get_pdata(struct arizona *arizona)
{
	...
	printk("%s: micd_detect_debounce=%d, micd_manual_debounce=%d, micd_bias_start_time=%d, micd_rate=%d, micd_dbtime=%d, micd_timeout=%d, micd_force_micbias=%d, micd_open_circuit_declare=%d, micd_low_ohm=%d\n", 
			__func__, pdata->micd_detect_debounce, pdata->micd_manual_debounce, pdata->micd_bias_start_time, pdata->micd_rate,pdata->micd_dbtime, pdata->micd_timeout, pdata->micd_force_micbias, pdata->micd_open_circuit_declare, pdata->micd_low_ohm);
...
static int arizona_extcon_probe(struct platform_device *pdev)
{
	...
	info->micd_low_ohm = pdata->micd_low_ohm;
	info->micd_low_ohm_done = false;
