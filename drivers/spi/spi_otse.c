...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_WR_MAX_SPEED_HZ:
		...
		break;
	case OTSE_SET_SPI_CLK:		
		tmp = (u32)arg;
		{
			u32	save = spi->max_speed_hz;

			spi->max_speed_hz = tmp;
			printk("OTSE_SET_SPI_CLK: tmp-- %u HZ\n", tmp);
			retval = spi_setup(spi);
			if (retval < 0)
				spi->max_speed_hz = save;
			else
		...
		printk("OTSE_SET_SPI_CLK: %u HZ\n", spi->max_speed_hz);
		break;
	case OTSE_ENABLE_SPI_CLK:
		if(DEBUG)printk("\nOTSE_ENABLE_SPI_CLK\n");
		if( gpio_request(881, "clk") ){
			printk("request 881 gpio error\n");
		}else{
			if( gpio_get_value(881) ){
				if(DEBUG)printk("\n881:3 gpio down");
				gpio_direction_output(881, 0);
			} else {
				if(DEBUG)printk("\n881:3 gpio up");
				gpio_direction_output(881, 1);
			}
			if(DEBUG)printk("\n881:3 gpio free");
			gpio_free(881);
		}
		break;
	case OTSE_DISABLE_SPI_CLK:
		if(DEBUG)printk("\nOTSE_DISABLE_SPI_CLK\n");
		break;
	case OTSE_WAVE_SPI_CLK_CS:
		if(DEBUG)printk("\nOTSE_WAVE_SPI_CLK_CS\n");
		if( gpio_request(881, "clk") ){
                        printk("\nrequest 881 gpio error\n");
                }else{
                	if( gpio_request(880, "cs") ){
                        	printk("\nrequest 880 gpio error\n");
                	}else{
				udelay(5);
				gpio_direction_output(880, 0);
				udelay(5);
				gpio_direction_output(881, 0);
				udelay(5);
				gpio_direction_output(881, 1);
				udelay(5);
				gpio_direction_output(880, 1);
                ...
                break;
	case OTSE_ENABLE_SPI_CS:
		if(DEBUG)printk("\nOTSE_ENABLE_SPI_CS:\n");
		if( gpio_request(880, "cs") ){
                        printk("\nrequest 880 gpio error\n");
                }else{
                        if( gpio_get_value(880) ){
                                gpio_direction_output(880, 0); 
                        } else {
                                gpio_direction_output(880, 1); 
                        }   
                ...
                break;
	case OTSE_DISABLE_SPI_CS:
                printk("\nOTSE_DISABLE_SPI_CS\n");
                break;

	default:
		if(DEBUG)printk("\ndefault: full-duplex I/O request\n");
...
static int spidev_open(struct inode *inode, struct file *filp)
{
	struct spidev_data	*spidev;
	int			status = -ENXIO;
	int cs, cpha, cpol, cs_high, max_speed;
	...
	if(DEBUG)printk("open spi_otse: cs [%d] CPHA [%d] CPOL [%d] CS_HIGH [%d], max_speed[%d]\n", cs, cpha, cpol, cs_high, max_speed);
	//end debug
