...
static irqreturn_t bcm2079x_dev_irq_handler(int irq, void *dev_id)
{
	...
	{
		printk("bcm2079x-i2c:irq aquire wake lock\n");
		wake_lock(&bcm2079x_dev->wake_lock);
	}else
	{
		printk("bcm2079x-i2c:irq wake lock count = %d\n", wakelockcnt);
	}
	//printk("irq handler ( wake lock %d)...\n", wakelockcnt);
#endif
	//printk("bcm2079x_dev_irq_handler\n");
	spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	{
#else
	if(!gpio_get_value(bcm2079x_dev->irq_gpio))
	{
...
static int nfc_parse_dt(struct device *dev, struct bcm2079x_platform_data *pdata)
{
	...
	pdata->en_gpio = of_get_named_gpio(np, "brcm,pwr-gpio", 0);
	if ((!gpio_is_valid(pdata->en_gpio)))
		return -EINVAL;

	pdata->irq_gpio = of_get_named_gpio(np, "brcm,irq-gpio", 0);
	if ((!gpio_is_valid(pdata->irq_gpio)))
		return -EINVAL;

	pdata->wake_gpio = of_get_named_gpio(np, "brcm,wak-gpio", 0);
			   ...
			   const struct i2c_device_id *id)
{
	...
	//add by zhuangnk1 2015-4-9 for qcom clock BBCLK2 start
	bcm2079x_dev->clk_run = false;
	bcm2079x_dev->s_clk = clk_get(&bcm2079x_dev->client->dev, "ref_clk");
	if(bcm2079x_dev->s_clk != NULL) {
		printk("bcm2079x request BBCLK2_clock");
		clk_prepare_enable(bcm2079x_dev->s_clk);
		bcm2079x_dev->clk_run = true;
	}
	//end by zhuangnk1 2015-4-9 for qcom clock BBCLK2 end
...
static int __init bcm2079x_dev_init(void)
{
	...
	//ret = gpio_request(923, "nfc_irq_gpio");
        //if (ret) {
	//	printk("gpio failed %d",ret);
        //}else{
	//	printk("gpio successful");
	//}
