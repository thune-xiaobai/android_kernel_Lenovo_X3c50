...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1021B, 0x021b, 1, FPC1021_ROWS, FPC1021_COLUMNS, FPC102X_ADC_GROUP_SIZE, 5000, 1000},
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	error = (fpc1020->soft_reset_enabled) ?
			...
			fpc1020_gpio_reset(fpc1020);
         printk("fpc1020_reset111111111111111111111fpc1020->soft_reset_enabled = %d\n",fpc1020->soft_reset_enabled);
		      ...
		      fpc1020_reg_access_t *reg_data)
{
	...
	struct spi_transfer cmd = {
		.cs_change = 0,
		.delay_usecs = 0,
		.speed_hz = (u32)fpc1020->spi_freq_reg_khz * 1000u,
		.tx_buf = &(reg_data->reg),
		.rx_buf = NULL,
	...
	if (!reg_data->write) {
		if (target_little_endian) {
			...
			while (dst < reg_data->reg_size) {
				...
				printk("reg_data->dataptr[%d]= %0x",dst,temp_buffer[src]);
				src--;
				dst++;
			}
		} else {
...
int fpc1020_wake_up(fpc1020_data_t *fpc1020)
{
	...
	int status = fpc1020_read_status_reg(fpc1020);
         printk("fpc1020_wake_up power = %d,reset  = %d,status = %d\n",power,reset,status);
