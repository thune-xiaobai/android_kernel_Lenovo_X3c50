...
static spinlock_t lock_count;
static int boost_touch_count = 0;
static int boost_count = 0;

struct synaptics_rmi4_data *syna_data;
...
int synaptics_rmi4_sensor_nosleep(bool enable);
int gesture_flag;
extern int LCD_ID;
static int syna_resume_flag = 0;
#ifdef CONFIG_FB
...
static ssize_t synaptics_rmi4_gesture_only_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
#endif
#if SYNA_GLOVE_MODE
static void synaptics_rmi4_f12_glove(struct synaptics_rmi4_data *rmi4_data,
		bool enable);
//static int synaptics_rmi4_f12_glove_enabled(struct synaptics_rmi4_data *rmi4_data);
static ssize_t synaptics_rmi4_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#endif
static ssize_t synaptics_rmi4_delay_ms_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_delay_ms_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_debug_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_debug_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_gpio_reset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t synaptics_rmi4_power_reset_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(gesture_only, S_IWUGO,
			synaptics_rmi4_show_error,
			synaptics_rmi4_gesture_only_store),
...
#endif
	__ATTR(debug_switch, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_debug_show,
			synaptics_rmi4_debug_store),
	__ATTR(delay_ms, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_delay_ms_show,
			synaptics_rmi4_delay_ms_store),
	__ATTR(gpio_reset, (S_IRUGO | S_IWUGO),
			NULL,
			synaptics_rmi4_gpio_reset_store),
	__ATTR(power_reset, (S_IRUGO | S_IWUGO),
			NULL,
			synaptics_rmi4_power_reset_store),
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (atomic_read(&rmi4_data->st_enabled)) {
		printk("%s,secure_touch is enabled,return\n",__func__);
		return -EINVAL;
	}
#endif
	if (sscanf(buf, "%u", &input) != 1)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	input = input > 0 ? 1 : 0;
	printk("%s,enable_wakeup_gesture:%d,input:%d\n",__func__,rmi4_data->enable_wakeup_gesture,input);
	if ((rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) && (rmi4_data->enable_wakeup_gesture != input))
	{
...
#if SYNA_GLOVE_MODE
static void synaptics_rmi4_glove_work(struct work_struct *work)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			...
			glove_work);
	printk("%s\n",__func__);
	//wait_for_completion(&rmi4_data->resume_processed);
	synaptics_rmi4_f12_glove(rmi4_data,rmi4_data->glove_enabled);
	return;
}
static ssize_t synaptics_rmi4_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	//rmi4_data->glove_enabled = synaptics_rmi4_f12_glove_enabled(rmi4_data);	
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	input = input > 0 ? 1 : 0;
	rmi4_data->glove_enabled = input;
	if(rmi4_data->suspend)
	queue_delayed_work(rmi4_data->glove_workqueue,
			&rmi4_data->glove_work,
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if (detected_gestures) {
			printk("%s,detected_gestures:%d\n",__func__,detected_gestures);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_WAKE, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_GESTURE_WAKE, 0);
			input_sync(rmi4_data->input_dev);
		//	rmi4_data->suspend = false;
		}
...
static int set_boost_touch(const char *buf, const struct kernel_param *kp)
{
    int value;
	unsigned long flags;
    ...
    spin_lock_irqsave(&lock_count, flags);
    boost_touch_count = value;
    spin_unlock_irqrestore(&lock_count, flags);
    return 0;
}

static int get_boost_touch(char *buf, const struct kernel_param *kp)
{
    int cnt = 0;
	unsigned long flags;
...
static const struct kernel_param_ops param_ops_boost_touch = {
	.set = set_boost_touch,
	.get = get_boost_touch,
};
module_param_cb(boost_touch, &param_ops_boost_touch, NULL, 0644);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
            ...
            if((x != prev_x) && boost_count && (fingers_to_process == 1))
            {
                if(skip_point)
                {
                    input_report_abs(rmi4_data->input_dev,
			...
			touch_count++;
			if((!touch_flag[finger]) && debug_switch)
			{
			printk("syna,finger:%d,status:%d\n",finger,finger_status);
			touch_flag[finger]= 1;
			}
			break;
		...
		unsigned int intr_count)
{
	...
	rmi4_data->f54_query_base_addr = fhandler->full_addr.query_base;
	rmi4_data->f54_ctrl_base_addr = fhandler->full_addr.ctrl_base;
	rmi4_data->f54_data_base_addr = fhandler->full_addr.data_base;
	rmi4_data->f54_cmd_base_addr = fhandler->full_addr.cmd_base;
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			dev_dbg(rmi4_data->pdev->dev.parent,
					...
					page_number);
		//	printk("syna:fn_number:%d,intr_src_count:%d\n",rmi_fd.fn_number,rmi_fd.intr_src_count);
			...
			case SYNAPTICS_RMI4_F54:
				...
				retval = synaptics_rmi4_f54_init(rmi4_data,fhandler,&rmi_fd,intr_count);
				if (retval < 0)
					return retval;
				}
				break;
...
flash_prog_mode:
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry(fhandler, &rmi->support_fn_list, link) {
			//	printk("syna:number of data source:%d,intr_reg_num:%d,fhandler intr_mask:%x\n",fhandler->num_of_data_sources,fhandler->intr_reg_num,fhandler->intr_mask);
			if (fhandler->num_of_data_sources) {
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->enable_wakeup_gesture = gesture_flag;
	else
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture) {
		set_bit(KEY_GESTURE_WAKE, rmi4_data->input_dev->keybit);
		input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_WAKE);
	}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	rmi4_data->resume_workqueue =
			create_singlethread_workqueue("dsx_resume_workqueue");
	INIT_DELAYED_WORK(&rmi4_data->resume_work, synaptics_rmi4_resume_work);
	rmi4_data->glove_workqueue =
			create_singlethread_workqueue("dsx_glove_workqueue");
	INIT_DELAYED_WORK(&rmi4_data->glove_work, synaptics_rmi4_glove_work);
...
//	init_completion(&rmi4_data->resume_processed);
#ifdef FB_READY_RESET
...
static int synaptics_rmi4_remove(struct platform_device *pdev)
{
	...
	flush_workqueue(rmi4_data->resume_workqueue);
	destroy_workqueue(rmi4_data->resume_workqueue);
	flush_workqueue(rmi4_data->glove_workqueue);
	destroy_workqueue(rmi4_data->glove_workqueue);
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		sysfs_remove_file(&rmi4_data->input_dev->dev.kobj,
				&attrs[attr_count].attr);
	}
...
static int synaptics_rmi4_f12_glove_enabled(struct synaptics_rmi4_data *rmi4_data)
{
	...
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	offset = extra_data->ctrl23_offset;
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f12_ctrl23 data\n",
				__func__);
		return -1;
	}

	printk("%s,address:0x%x,glove_enable:0x%x\n",__func__,fhandler->full_addr.ctrl_base + offset,glove_enable);
	glove_enable &= 0x20;
	return (glove_enable ? 1:0);
}
#endif
static void synaptics_rmi4_f12_glove(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval;
	unsigned char offset_23,offset_26;
	unsigned char glove_enable,f12_ctrl_26;
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
	...
	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	offset_23 = extra_data->ctrl23_offset;
	offset_26 = extra_data->ctrl26_offset;
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + offset_23,
			&glove_enable,
			sizeof(glove_enable));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f12_ctrl23\n",
				__func__);
		return;
	}
	retval = synaptics_rmi4_reg_read(rmi4_data,
			fhandler->full_addr.ctrl_base + offset_26,
			&f12_ctrl_26,
			sizeof(f12_ctrl_26));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read f12_ctrl26\n",
				__func__);
		return;
	}
	...
	printk("%s,address:0x%x,set glove_enable to:0x%x,address:0x%x to 0x%x\n",__func__,fhandler->full_addr.ctrl_base + offset_23,glove_enable,fhandler->full_addr.ctrl_base + offset_26,f12_ctrl_26);
	retval = synaptics_rmi4_reg_write(rmi4_data,
			fhandler->full_addr.ctrl_base + offset_23,
			&glove_enable,
			sizeof(glove_enable));
...
#endif
int synaptics_rmi4_sensor_forcal(void)
{
	int retval;
	unsigned char device_ctrl;
	if(!syna_data)
		return -1;
	retval = synaptics_rmi4_reg_read(syna_data,
			syna_data->f54_cmd_base_addr,
			&device_ctrl,
			sizeof(device_ctrl));
	device_ctrl |= 0x02;
		retval = synaptics_rmi4_reg_write(syna_data,
				syna_data->f54_cmd_base_addr,
				&device_ctrl,
				sizeof(device_ctrl));
	retval = synaptics_rmi4_reg_read(syna_data,
...
{
	int retval;
	unsigned char device_ctrl = 0x04;
	if(!syna_data)
		return -1;
	printk("%s\n",__func__);
	if(enable)
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,"%s\n",__func__);
	if (rmi4_data->stay_awake)
	{
		return 0;
	}
	if (rmi4_data->enable_wakeup_gesture) {
		synaptics_rmi4_wakeup_gesture(rmi4_data, true);
		synaptics_rmi4_sensor_nosleep(false);
		if(LCD_ID == 1)
		{
		msleep(delay_ms);
		synaptics_rmi4_sensor_forcal();
		}
		goto exit;
	}
	...
	mutex_unlock(&exp_data.mutex);
	//pull down reset gpio,disable regulator
	printk("%s,disable regulator and gpio\n",__func__);
	if (bdata->reset_gpio >= 0) {
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		msleep(bdata->reset_active_ms);
	}
...
static void synaptics_rmi4_resume_work(struct work_struct *work)
{
	...
	if (rmi4_data->stay_awake)
	{
		if(syna_resume_flag <2)
			syna_resume_flag++;
//		complete(&rmi4_data->resume_processed);
		return ;
	}
	if (rmi4_data->enable_wakeup_gesture) {
		synaptics_rmi4_wakeup_gesture(rmi4_data, false);
	...
	printk("%s,enable reg and gpio\n",__func__);
	//enable regulator,pull up reset gpio pin
	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
	if (retval < 0) {
		printk("%s: Failed to enable regulators\n",
				__func__);
...
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	queue_delayed_work(rmi4_data->resume_workqueue,
			&rmi4_data->resume_work,
			0);
	return 0;
}
