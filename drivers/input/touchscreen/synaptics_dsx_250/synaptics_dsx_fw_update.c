...
#define FW_IMAGE_NAME_TM "synaptics/TMstartup_fw_update.img"
...
static ssize_t synaptics_rmi4_touch_info_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(ic_info, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(touch_info, S_IRUGO,
			synaptics_rmi4_touch_info_show,
			synaptics_rmi4_store_error),
};
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_TM, sizeof(FW_IMAGE_NAME_TM),
				sizeof(FW_IMAGE_NAME_TM));
		else
		{
			printk("syna:unknown vendor id,use boe\n");
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME, sizeof(FW_IMAGE_NAME),
				sizeof(FW_IMAGE_NAME));
		}
...
exit:
	...
	dev_err(rmi4_data->pdev->dev.parent,"%s: End of reflash process\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	int SYNA_FW_VERSION,SYNA_VENDOR_ID;
//	unsigned char vendor = 0;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			config_id,4);
	if (retval < 0) {
		printk("%sFailed to read configID from device\n",__func__);
	} 

	printk("configid %d, %d, %d,%d\n", config_id[0], config_id[1],config_id[2],config_id[3]);
	SYNA_FW_VERSION = config_id[3];
	SYNA_VENDOR_ID = config_id[0];
	...
	return snprintf(buf, PAGE_SIZE, "synaptics_%s_%s_0x%x\n",rmi4_data->rmi4_mod_info.product_id_string, vendor_id, SYNA_FW_VERSION);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "synapcitcs fw version 0x%02x 0x%02x 0x%02x 0x%02x\n", config_id[0], config_id[1],config_id[2], config_id[3]);
}
