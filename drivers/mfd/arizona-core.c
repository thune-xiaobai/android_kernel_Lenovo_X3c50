...
static unsigned int arizona_chip_id = 0;
static struct kernel_param_ops arizona_chip_id_param_ops = {
	.set = param_set_int,
	.get = param_get_int,
};

module_param_cb(arizona_chip_id, &arizona_chip_id_param_ops, &arizona_chip_id, 0644);


#ifdef CONFIG_SND_SOC_FLORIDA
#if 0
static int hp_switch_ent_selcet_gpio;
static int hp_switch_ent_mute_gpio;
void arizona_Hpout_swtich_to_florida(void)
{
  if (hp_switch_ent_selcet_gpio){
        gpio_set_value_cansleep(hp_switch_ent_selcet_gpio, 0);
    }
  if (hp_switch_ent_mute_gpio){
        gpio_set_value_cansleep(hp_switch_ent_mute_gpio, 0);
    }    
}
EXPORT_SYMBOL_GPL(arizona_Hpout_swtich_to_florida);

void arizona_Hpout_swtich_to_Ess(void)
{
  if (hp_switch_ent_selcet_gpio){
        gpio_set_value_cansleep(hp_switch_ent_selcet_gpio, 1);
    }
  if (hp_switch_ent_mute_gpio){
        gpio_set_value_cansleep(hp_switch_ent_mute_gpio, 0);
    }    
}
EXPORT_SYMBOL_GPL(arizona_Hpout_swtich_to_Ess);
#endif
#endif
...
static int arizona_runtime_resume(struct device *dev)
{
	...
	dev_dbg(arizona->dev, "Leaving AoD mode Complete\n");
	return 0;
...
static int arizona_runtime_suspend(struct device *dev)
{
	...
	dev_dbg(arizona->dev, "Entering AoD mode complete\n");
	return 0;
err:
...
#if 0
    pdata->hp_power_gpio = arizona_of_get_named_gpio(arizona, "wlf,hp_power_switch_gpio", true);

    pdata->hp_switch_1_gpio = arizona_of_get_named_gpio(arizona, "wlf,hp_switch_1_gpio", true);

    pdata->hp_switch_2_gpio = arizona_of_get_named_gpio(arizona, "wlf,hp_switch_2_gpio", true);
#endif
    
    
    arizona_of_read_u32(arizona, "wlf,irq_flags", false, &pdata->irq_flags);
...
#if 0
    dev_err(dev, "arizona->pdata.hp_power_gpio=%d,arizona->pdata.hp_switch_1_gpio=%d arizona->pdata.hp_switch_2_gpio=%d\n", arizona->pdata.hp_power_gpio,arizona->pdata.hp_switch_1_gpio,arizona->pdata.hp_switch_2_gpio);


    if (arizona->pdata.hp_power_gpio) {
		/* Start out with /RESET low to put the chip into reset */
		ret = gpio_request_one(arizona->pdata.hp_power_gpio,
				       GPIOF_DIR_OUT | GPIOF_INIT_HIGH,
				       "arizona /HP_Power");
		if (ret != 0) {
			dev_err(dev, "Failed to request /HP_Power: %d\n", ret);
			goto err_notifier;
		}
	}

    
    if (arizona->pdata.hp_switch_1_gpio) {
		/* Start out with /RESET low to put the chip into reset */
		ret = gpio_request_one(arizona->pdata.hp_switch_1_gpio,
				       GPIOF_DIR_OUT | GPIOF_INIT_LOW,
				       "arizona /HP_Power");
		if (ret != 0) {
			dev_err(dev, "Failed to request /HP_Power: %d\n", ret);
     ...
     hp_switch_ent_selcet_gpio= arizona->pdata.hp_switch_1_gpio;
    if (arizona->pdata.hp_switch_2_gpio) {
		/* Start out with /RESET low to put the chip into reset */
		ret = gpio_request_one(arizona->pdata.hp_switch_2_gpio,
				       GPIOF_DIR_OUT | GPIOF_INIT_LOW,
				       "arizona /HP_Power");
		if (ret != 0) {
			dev_err(dev, "Failed to request /HP_Power: %d\n", ret);
     ...
     hp_switch_ent_mute_gpio= arizona->pdata.hp_switch_2_gpio;
#endif        
